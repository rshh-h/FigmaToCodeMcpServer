...
const swiftuiWidgetGenerator = (
  ...
  visibleSceneNode.forEach((node) => {
    ...
    if (localVectorPath) {
      comp.push(swiftuiLocalAsset(node, localVectorPath));
      return;
    }
...
const swiftuiLocalAsset = (node: SceneNode, assetPath: string): string =>
  swiftuiContainer(node, `Text("${assetPath}")`);
