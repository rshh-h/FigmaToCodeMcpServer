...
const flutterWidgetGenerator = (
  ...
  visibleSceneNode.forEach((node) => {
    ...
    if (localVectorPath) {
      comp.push(flutterLocalAsset(node, localVectorPath, true));
      return;
    }
...
const flutterLocalAsset = (
  node: SceneNode,
  assetPath: string,
  isSvg: boolean,
): string => {
  const assetWidget = isSvg
    ? `SvgPicture.asset('${assetPath}', width: ${Math.round(node.width)}, height: ${Math.round(node.height)})`
    : `Image.asset('${assetPath}', width: ${Math.round(node.width)}, height: ${Math.round(node.height)}, fit: BoxFit.fill)`;
  return flutterContainer(node, assetWidget);
};
