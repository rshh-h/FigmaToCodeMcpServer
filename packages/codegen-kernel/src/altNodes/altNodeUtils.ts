...
export const getLocalImagePath = (node: SceneNode): string | undefined =>
  (node as SceneNode & { localImagePath?: string }).localImagePath;

export const getLocalVectorPath = (node: SceneNode): string | undefined =>
  (node as SceneNode & { localVectorPath?: string }).localVectorPath;

export const isLocalVectorChildNode = (node: SceneNode): boolean =>
  (node as SceneNode & { isLocalVectorChild?: boolean }).isLocalVectorChild === true;

export const renderAndAttachSVG = async (node: any) => {
  if (node.localVectorPath) {
    node.svg = "";
    return node;
  }
